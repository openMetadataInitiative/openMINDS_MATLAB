function versionNum = getModelVersion(outputType)
    ...
    if toc(lastTic) > 1
        ...
        if numel(versionNames) > 1
            warning('openMINDS:MultipleModelVersionsOnPath', ...
                ['Multiple openMINDS model versions are on the search path: ', ...
                '%s. Types have the same names in every version, so they ', ...
                'conflict. Call openminds.selectModelVersion to put one ', ...
                'model version, and only that one, on the search path.'], ...
                strjoin(versionNames, ', '));
        end
