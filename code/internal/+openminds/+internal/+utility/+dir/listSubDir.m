...
%   [absPath, dirName] = listSubDir(rootPath, expression, ignoreList, nRecurse)
    ...
    import openminds.internal.utility.dir.listSubDir % self
    ...
    if isa(rootPath, 'cell') || isa(rootPath, 'string') && strlength(rootPath) > 1
        for i = 1:numel(rootPath)
            [absPathIter, dirNameIter] = listSubDir(rootPath{i}, expression, ignoreList, nRecurse);
        ...
        if nRecurse > 0 && sum(keep) > 0 % Todo:
            [absPathRec, dirNameRec] = listSubDir(absPath, expression, ignoreList, nRecurse-1);
